...
float4 frag (Interpolators i) : SV_Target {
    ...
    float3 N = mul( mtxTangToWorld, tangentSpaceNormal );
        ...
        #ifdef IS_IN_BASE_PASS
            float fresnel =  _F0 + (1-_F0) * pow(1-saturate(dot(V,N)),5);
            float3 viewRefl = reflect( -V, N );
            float mip = (1-_Gloss)*8;
		...
		float3 color = diffuseLight * surfaceColor * lerp(saturate(SHADOW_ATTENUATION(i)), 1, _Shadow) * (1-_F0) + specularLight; 
