...
#pragma warning restore 0649
        ...
        public void Execute(int index)
        {
            ...
            // Goal
            float3 forceGoal = new float3(0, 0, 0);
            if (goal != null)
            {
                Vector3 distGoal = goal - new Vector3(position.x, position.y, position.z);
                float3 normDist = new float3(distGoal.normalized.x, distGoal.normalized.y, distGoal.normalized.z);
                float goalPower = 100000.0f;
                forceGoal = normDist * goalPower;
                //Debug.Log("force goal: %f, %f, %f" + forceGoal.x + forceGoal.y + forceGoal.z);
                //Debug.Log("force gravity:" + forceGravity.x + forceGravity.y + forceGravity.z);
            }

            // Apply
            particlesForces[index] = forcePressure + forceViscosity + forceGravity + forceGoal;
            //particlesForces[index] = forceGravity + forceGoal;
        }
    }
...
#pragma warning restore 0649
    ...
    [BurstCompile]
    private struct ApplyTranslations
    {
        [ReadOnly] public NativeArray<Translation> particlesPosition;
        [ReadOnly] public NativeArray<SPHVelocity> particlesVelocity;
    ...
    private JobHandle OnUpdate(JobHandle inputDeps)
    {
        if (GameObject.FindWithTag("Goal") != null)
            goalPosition = GameObject.FindWithTag("Goal").transform.position;
